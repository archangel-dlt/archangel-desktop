...
contract Archangel {
  ...
  function hasPermission(address addr) external view returns(bool) {
    return bytes(permissions[addr]).length != 0;
  } // hasPermissions

  function grantPermission(address addr, string calldata name) external ownerOnly {
  ...
  function store(string calldata key, string calldata payload) external permittedOnly {
  ...
  function fetch(string calldata key) external view returns(string memory, bytes32) {
    Payload storage payload = registry[key];
    return (payload.payload, payload.previous_key);
  } // fetch

  function fetchPrevious(bytes32 key) external view returns(string memory, bytes32) {
    Payload storage payload = previous_versions[key];
    return (payload.payload, payload.previous_key);
  } // fetchPrevious

  //////////////////////////////////
  function moveExisting(string memory key) private returns (bytes32) {
    bytes32 previous_key;
